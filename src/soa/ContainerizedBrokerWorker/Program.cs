    ...
    internal static class Program
    {
        ...
        private static int Main(string[] args)
        {
           ...
           // int pid = Process.GetCurrentProcess().Id;
            ...
            Trace.TraceInformation("Current worker hostname is " + workerHostName);
            ...
            try
            {
                Uri brokerManagementServiceAddress = new Uri(SoaHelper.NewContainerizedBrokerManagementServiceAddress());
                Trace.TraceInformation("Current worker management service address is " + brokerManagementServiceAddress.ToString());
                ServiceHost host;
                ...
                try
                {
                    ...
                    //trace.LogBrokerWorkerMessage(workerHostName, string.Format("[Main] Try open broker management service at {0}.", brokerManagementServiceAddress.ToString()));
                ...
                {
                    //trace.LogBrokerWorkerUnexpectedlyExit(pid, string.Format("[Main] Failed to open broker management service: {0}", e));
                ...
                {
                    //trace.LogBrokerWorkerMessage(pid, string.Format(CultureInfo.InvariantCulture, "[Program].Main: Exception {0}", ex1));

                    //trace.LogBrokerWorkerMessage(pid, "[Main] Failed to close the ServiceHost.");
                    ...
                    {
                        //trace.LogBrokerWorkerMessage(pid, string.Format(CultureInfo.InvariantCulture, "[Program].Main: Exception {0}", ex));

                        //trace.LogBrokerWorkerMessage(pid, "[Main] Failed to abort the ServiceHost.");
                    }
                }
