...
namespace Microsoft.Telepathy.Internal.ContainerizedBrokerLauncher
{
    ...
    /// </summary>
    internal class ContainerizedBrokerProcessPool : IDisposable
    {
        /// <summary>
        ...
        /// </summary>
        private List<ContainerizedBrokerProcess> pool;
        ...
        /// <summary>
        /// Stores the containerized context
        /// </summary>
        ...
        /// </summary>
        /// <param name="context">indicating the containerized context</param>
        public ContainerizedBrokerProcessPool(ITelepathyContext context)
        {
            this.context = context;
            this.currentPoolSize = this.poolSize;
            this.pool = new List<ContainerizedBrokerProcess>(this.poolSize);
        ...
        /// </summary>
        ~ContainerizedBrokerProcessPool()
        {
            this.Dispose(false);
        }
        ...
        public ContainerizedBrokerProcess GetBrokerProcess()
        {
                ...
                if (this.pool.Count > 0)
                {
                    lock (this.pool)
                    {
                        if (this.pool.Count > 0)
                        {
                            ...
                            ContainerizedBrokerProcess process = this.pool[this.pool.Count - 1];
        ...
        private void Dispose(bool disposing)
        {
            ...
            lock (this.pool)
            {
                foreach (ContainerizedBrokerProcess process in this.pool)
                {
                    process.Close();
                }
        ...
        {
            ContainerizedBrokerProcess process = new ContainerizedBrokerProcess(context, null);
        ...
        {
            ContainerizedBrokerProcess process = (ContainerizedBrokerProcess)sender;
