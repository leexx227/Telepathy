...
namespace Microsoft.Telepathy.Internal.ContainerizedBrokerLauncher
{
    ...
    /// <summary>
    /// Manager for all containerized broker app domains
    /// </summary>
    /// <remarks>
    /// Locking order:
    ...
    /// </remarks>
    internal sealed class ContainerizedBrokerManager : ISessionUserAuthenticator, IDisposable
    {
        /// <summary>
        ...
        /// </summary>
        private volatile Dictionary<string, ContainerizedBrokerInfo> brokerDic;
        ...
        /// </summary>
        private volatile ContainerizedBrokerProcessPool pool;
        ...
        /// </summary>
        public ContainerizedBrokerManager(bool needRecover, ITelepathyContext context)
        {
            ...
            this.brokerDic = new Dictionary<string, ContainerizedBrokerInfo>();
...
#endif
            //TODO: Disable BrokerProcessPool Temporarily
            //this.pool = new ContainerizedBrokerProcessPool(context);
        ...
        public async Task<BrokerInitializationResult> CreateNewBrokerDomain(SessionStartInfoContract info, string sessionid, bool durable)
        {
            ...
            {
                this.schedulerHelper = SchedulerHelperFactory.GetSchedulerHelper(this.context, sessionid);
            }
        ...
        public async Task CloseBrokerDomain(string sessionId, string brokerName)
        {
            TraceHelper.TraceEvent(sessionId, System.Diagnostics.TraceEventType.Information, "[BrokerManager] Close broker {0}", sessionId);

            ContainerizedBrokerInfo info = null;
            lock (this.brokerDic)
            {
                this.brokerDic.TryGetValue(sessionId, out info);
            }
            if (info == null && !string.IsNullOrEmpty(brokerName))
            {
                await this.CleanupContainersAsync(sessionId, brokerName, false);
            }
            if (info != null)
            {
                info.CheckAccess();
        ...
        private static void RevertCreateDomainAndRun(ContainerizedBrokerInfo info, bool suspended)
        {
            if (info == null)
            {
                return;
        ...
        private async Task CleanupContainersAsync(string sessionId, string brokerName, bool suspend)
        {
            // lock the broker dic to remove the info
            // if no such broker info is found, return because somebody has done the cleanup.
            int step = 0;
            ...
            try
            {
                ContainerizedBrokerInfo.CloseContainerizedBroker(suspend, brokerName, sessionId);
                TraceHelper.TraceEvent(sessionId, System.Diagnostics.TraceEventType.Information, "[BrokerManager] Cleanup: Step {0}: Close broker succeeded", ++step);
            }
            catch (Exception e)
            {
        ...
        {
            // Check the brokerDic to see if the session Id already exists, get all brokerDic from all pods
            // TODO: should be check by querying redis 
           /* lock (this.brokerDic)
            {
            ...
            string serviceRegistrationFileName =
                ServiceRegistrationRepo.GetServiceRegistrationFileName(recoverInfo.StartInfo.ServiceName,
                    recoverInfo.StartInfo.ServiceVersion);
       
            // Get service registration file content from Redis and save in the specified path 
            string serviceRegistrationFileContent =
               await context.Registry.GetValueAsync<string>(null, serviceRegistrationFileName, new CancellationToken());

            string serviceRegistrationPath = serviceRegistration.GetBrokerLauncherServiceRegistrationPath(serviceRegistrationFileName, serviceRegistrationFileContent);
            TraceHelper.TraceEvent(TraceEventType.Verbose, "[BrokerManager] .CreateBrokerAndRun: Get file {0} from Redis, save in the current container path {1}", serviceRegistrationFileName, serviceRegistrationPath);
            
            if (serviceRegistrationFileContent == null)
            {
            ...
            ContainerizedBrokerInfo info = new ContainerizedBrokerInfo(recoverInfo, brokerInfo, auth, customBroker, this.pool, context);
            BrokerTracing.TraceVerbose("[BrokerManager].CreateBrokerAndRun: Set BrokerInfo object in Redis for future use, like close brokerworker and so on");
            await context.Registry.SetValueAsync<ContainerizedBrokerInfo>(null, "brokerinfo" + recoverInfo.SessionId, info, new CancellationToken());
            BrokerTracing.TraceVerbose("[BrokerManager].CreateBrokerAndRun: Set BrokerInfo object in Redis successfully, the key is {0}", "brokerinfo" + recoverInfo.SessionId);
        ...
        {
            ContainerizedBrokerInfo info = (ContainerizedBrokerInfo)sender;
        ...
        private async Task RecoverThreadProc(CancellationToken ct)
        {
            ...
            this.staleSessionCleanupTimer.Change(0, ContainerizedBrokerManager.StaleSessionCleanupPeriod);

            if (this.updateQueueLengthTimer != null)
            {
                // TODO: on azure, about the MSMQ. Don't use the MSMQ in the Azure cluster.
                this.updateQueueLengthTimer.Change(0, ContainerizedBrokerManager.UpdateQueueLengthPeriod);
            }
        ...
        public bool? IfSessionCreatedByAadOrLocalUser(string sessionId)
        {
            ContainerizedBrokerInfo info = null;
            lock (this.brokerDic)
            {
