...
namespace Microsoft.Telepathy.Internal.ContainerizedBrokerLauncher
{
    ...
    internal class SchedulerHelper : ISchedulerHelper
    {
        ...
        /// </summary>
        public SchedulerHelper(ITelepathyContext context, string sessionId)
        {
            this.context = context;
            this.currentSessionId = sessionId;
            this.sessionNode = new Lazy<string>(() => this.ResolveSessionNodeWithRetries(sessionId).GetAwaiter().GetResult(), LazyThreadSafetyMode.ExecutionAndPublication);
        ...
        public async Task UpdateBrokerInfo(ContainerizedBrokerInfo info)
        {
                ...
                async (e, r) =>
                    {
                        ...
                        await this.RenewSchedulerAdapterClientAsync(jobid);
                    },
                retry);
        }
                       ...
                       async (e, r) =>
                           {
                               TraceHelper.TraceEvent(
                                   ...
                                   r.RetryCount,
                                   e);
                               await this.RenewSessionLauncherClientAsync(currentSessionId);
                           },
                       retry);
        }
                       ...
                       async () =>
                           {
                               using (BrokerIdentity identity = new BrokerIdentity())
                               {
                                   TraceHelper.TraceEvent(TraceEventType.Verbose, "[SchedulerHelper] Contact with session node: {0}", sessionNode.Value);
                                   identity.Impersonate();
        ...
        /// </summary>
        private async Task RenewSchedulerAdapterClientAsync(string sessionId)
        {
            if (this.schedulerClient.IsValueCreated)
            {
        ...
        /// Renew client proxy to scheduler adapter internal service
        /// </summary>
        private async Task RenewSessionLauncherClientAsync(string sessionId)
        {
            if (this.sessionLauncherClient.IsValueCreated)
            {
                ...
                async (e, r) =>
                    {
                        ...
                        await this.RenewSchedulerAdapterClientAsync(currentSessionId).ConfigureAwait(false);
                    },
        ...
        private async Task<string> ResolveSessionNodeWithRetries(string sessionId)
        {
            RetryManager retry = SoaHelper.GetDefaultInfinitePeriodRertyManager();
            return await RetryHelper<string>.InvokeOperationAsync(
                       async () => await this.context.ResolveSessionLauncherNodeAsync(sessionId),
                       async (e, r) =>
                           {
        ...
        private async Task<string> ResolveContainerizedSessionNodeWithRetries(string sessionId)
        {
            RetryManager retry = SoaHelper.GetDefaultInfinitePeriodRertyManager();
            return await RetryHelper<string>.InvokeOperationAsync(
                       async () => await this.context.ResolveSessionLauncherNodeAsync(sessionId),
