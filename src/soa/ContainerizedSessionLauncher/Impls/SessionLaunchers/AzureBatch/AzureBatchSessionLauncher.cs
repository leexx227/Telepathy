            ...
            string hostpath)
        {
            try
            {
                ...
                using (var batchClient = AzureBatchConfiguration.GetBatchClient())
                {
                    ...
                    TraceHelper.TraceEvent(TraceEventType.Verbose, "[AzureBatchSessionLauncher] .CreateAndSubmitSessionJob: New Created AzureBatch jobId={0}.", jobId);
                    ...
                    if (brokerPerfMode)
                    {
                        var brokerLoadBalanceServiceName = Environment.GetEnvironmentVariable(TelepathyConstants.BrokerLauncherAddress);
                        IClusterContext clusterContext = TelepathyContainerContext.Get().ClusterContext;
                        IKubernetes kubeClient = await clusterContext.GetClusterClient<IKubernetes>();
                        V1Service brokerLauncher = await kubeClient.ReadNamespacedServiceAsync(brokerLoadBalanceServiceName, "telepathy");
                        string brokerLauncherAddress = brokerLauncher.Status.LoadBalancer.Ingress[0].Ip;
                        sessionAllocateInfo.BrokerLauncherEpr = new[] { SoaHelper.GetBrokerLauncherAddress(brokerLauncherAddress) };
                        TraceHelper.TraceEvent(TraceEventType.Verbose, "[AzureBatchSessionLauncher] .CreateAndSubmitSessionJob: Get BrokerLauncherEpr={0}.", sessionAllocateInfo.BrokerLauncherEpr[0]);
                        string sessionLauncherAddress = Environment.GetEnvironmentVariable(TelepathyConstants.SessionLauncherAddress);
                        TraceHelper.TraceEvent(TraceEventType.Verbose, "[AzureBatchSessionLauncher] .CreateAndSubmitSessionJob: Set SessionId={0} and SessionLauncherEpr={1} mapping info in Redis.", sessionId, sessionLauncherAddress);
                        await clusterContext.Registry.SetValueAsync<string>(null, sessionId, sessionLauncherAddress, new CancellationToken());
                        string testRes = await clusterContext.Registry.GetValueAsync<string>(null, sessionId, new CancellationToken());
                        TraceHelper.TraceEvent(TraceEventType.Verbose, "[AzureBatchSessionLauncher] .CreateAndSubmitSessionJob: Get SessionLauncherEpr={0} mapping info from Redis.", testRes);
