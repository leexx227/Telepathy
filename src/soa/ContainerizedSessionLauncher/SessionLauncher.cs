...
namespace Microsoft.Telepathy.Internal.SessionLauncher
{
    ...
    internal abstract class SessionLauncher : DisposableObject, ISessionLauncher
    {
        ...
        protected virtual async Task<SessionAllocateInfoContract> AllocateInternalAsync(SessionStartInfoContract startInfo, string endpointPrefix, bool durable)
        {
            ...
            // If the serviceConfigFile wasn't been found and serviceversion isn't specified, try getting the service config based on the service's latest version
            ...
            // Put configuration file content in Redis to be shared cross cluster
            try{
                TraceHelper.TraceEvent(TraceEventType.Verbose, "[SessionLauncher] Try to get redis registry.");
                IRegistry registry = TelepathyContainerContext.Get().ClusterContext.Registry;
                await registry.SetValueAsync(null, serviceName, File.ReadAllText(serviceConfigFile), new CancellationToken());
                TraceHelper.TraceEvent(TraceEventType.Verbose, "[SessionLauncher] Put the service registration file in redis registry successfully.");
            }
            catch(Exception ex)
            {
                TraceHelper.TraceEvent(TraceEventType.Error, "Error happens when put service registration file in Redis: " + ex.ToString());
            }      
